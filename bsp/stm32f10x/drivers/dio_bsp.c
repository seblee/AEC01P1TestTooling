...
const pin_map_st in_pin_map_inst[Pin_Map_In]=//Êý×ÖÊäÈëPin_Map
{
...
const pin_map_st out_pin_map_inst[Pin_Map_Out]=//Êý×ÖÊä³öPin_Map
{
...
////PWMµç¼ÓÈÈ¶¨Ê±Æ÷ÖÐ¶Ï»Øµ÷º¯Êý,¸ÄÎª1Â·
...
//				rt_kprintf("di_reg[0] = %d,di_reg[1] = %d,di_reg[2] = %d,di_reg[3] = %d,di_reg[4] = %d\n",di_reg[0],di_reg[1],di_reg[2],di_reg[3],di_reg[4]);
...
/**
  ...
  */
//Êý×ÖÊäÈëº¯Êý£¬¶ÔËùÓÐÊý×ÖÊäÈë×´Ì¬½øÐÐ¸üÐÂ
...
//		for(i=0;i<=DI_MAX_CNT-1;i++)
//		{		
//				read_bitmap[i/16] |= GPIO_ReadInputDataBit(in_pin_map_inst[DI_MAX_CNT-1-i].pin_base,in_pin_map_inst[DI_MAX_CNT-1-i].pin_id);
//				read_bitmap[i/16] <<= 1;
...
/**
  ...
  * @retval none
  */
//Êý×ÖÊäÈë¶¨Ê±Æ÷»Øµ÷º¯Êý£¬¶ÔÊý×ÖÊäÈëµçÆ½½øÐÐ²ÉÑùºó·ÅÈë»º³å¶ÓÁÐ£»
...
/**
  ...
  * @retval none
  */
//Êý×ÖÊäÈë¶¨Ê±Æ÷£¬Ã¿6msÖÜÆÚ¶ÔÊý×ÖÊäÈë½øÐÐ²ÉÑù
