...
static int16_t get_average_temp(uint8_t type)
{
    ...
    if (type == TARGET_MODE_RETURN)
    {
        ...
        //				rt_kprintf("sum=%x,index= %x,MBM_DEV_A1_ADDR=
        //%x\n",sum,index,sys_get_mbm_online(MBM_DEV_A1_ADDR));
        //				//alair,20170224
        //				if((g_sys.config.general.cool_type == COOL_TYPE_COLUMN_WIND)&&(g_sys.config.compressor.type ==
        // COMP_QABP))		//列间变频
        //				{
        //						//回风2
        ...
        //				rt_kprintf("sum=%x,index= %x,MBM_DEV_A1_ADDR= %x,SENSOR_STS_REG_NO=
...
static int16_t calc_hi_press_ai(uint16_t adc_value, uint16_t k_factor, int16_t cali)
{
    ...
    ret_val = ((3.3 * adc_value * 100 * 4) / (4096 * 5) - 10) * 1000 / k_factor +
...
void ai_sts_update(sys_reg_st *gds_sys_ptr)
{
    ...
    // rt_kprintf("ADC1ConvertedValue = %X,V[1] = %X,V[2] = %X,V[3] = %X,V[4] = %X,V[5] =
...
//    }
    //		rt_kprintf("AI_NTC1 = %d,AI_NTC2 = %d,AI_NTC5 = %d,AI_NTC6 = %d,AI_NTC7 = %d,AI_NTC8 =
    ...
    //		rt_kprintf("ain[2] = %d,ain[3] = %d,ain[4] = %d,ain[5] = %d,ain[6] = %d,ain[7] =
