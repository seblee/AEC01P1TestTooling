...
static void BKG_Var_init(void)
{
    ...
    memset(g_sys.status.inv_compress_alarm.Inv_hi_Temperature, 0x00,
           sizeof(g_sys.status.inv_compress_alarm.Inv_hi_Temperature));
}
...
static void temp_hum_calc(void)
{
    ...
    //				rt_kprintf("return_air_hum=%x,return_air_temp=
...
static void ec_fan_diff_req(void)
{
    ...
    if (((team_local_inst.team_table[g_sys.config.team.addr - 1][TEAM_TAB_STATUS] & (0x0001 << TEAM_STS_CONFD_BPOS)) ==
         0) ||
        ...
        (g_sys.status.ain[AI_SENSOR1] != ABNORMAL_VALUE))
    {
        ...
        l_sys.ec_fan_diff_reg =
            lim_min_max((int16_t)(g_sys.config.fan.min_speed - g_sys.config.fan.set_speed),
                        (int16_t)(g_sys.config.fan.max_speed - g_sys.config.fan.set_speed), ec_fan_diff_tmp);
    }
    else
    {
...
static void ec_fan_suc_temp(void)
{
    ...
    if (((g_sys.config.dev_mask.ain) & (0x01 << AI_COMP_RETURN1)) &&
         ...
         (g_sys.config.general.cool_type == COOL_TYPE_COLUMN_WIND)))
    {
        ...
        else
        {
            ...
            switch (suc_temp_fsm)
            {
                ...
                case FSM_SUC_TEMP_DECRESS: {
                    if (g_sys.status.ain[AI_COMP_RETURN1] <=
                        (g_sys.config.fan.target_suc_temp - g_sys.config.fan.suc_temp_deadzone))
                    {
                        suc_temp_fsm = FSM_SUC_TEMP_IDLE;
                        suc_temp_reg = 0;
                    }
        ...
        l_sys.ec_fan_suc_temp =
            lim_min_max((int16_t)(g_sys.config.fan.min_speed - g_sys.config.fan.set_speed),
                        (int16_t)(g_sys.config.fan.max_speed - g_sys.config.fan.set_speed), suc_temp_reg);
    }
    else
    {
...
static void inv_compress_alarm_req(void)
{
    ...
    //			time_t now;
    //			get_local_time(&now);
    //			if((get_inv_comp_freq_down_signal(HIGH_PRESS) > 0) && (g_sys.status.inv_compress_alarm.inv_hipress_tmp ==
    //0))
    //			{
    //				g_sys.status.inv_compress_alarm.inv_alarm_counter ++;
    //				g_sys.status.inv_compress_alarm.inv_start_time[0] =
    //g_sys.status.inv_compress_alarm.inv_start_time[1]; 				g_sys.status.inv_compress_alarm.inv_start_time[1] =
    //g_sys.status.inv_compress_alarm.inv_start_time[2]; 				g_sys.status.inv_compress_alarm.inv_start_time[2] = now;
    ...
    //				}
    //
    //			if((get_inv_comp_freq_down_signal(HIGH_PRESS) == 0) && g_sys.status.inv_compress_alarm.inv_hipress_stop_flag
    //== 1)
    //			{
...
static void hipress_avg_calc(void)
{
    ...
    {
        g_sys.status.inv_compress_alarm.Inv_hi_Temperature[GET_ZERO] =
            g_sys.status.inv_compress_alarm.Inv_hi_Temperature[GET_ONE];
        g_sys.status.inv_compress_alarm.Inv_hi_Temperature[GET_ONE] =
            g_sys.status.inv_compress_alarm.Inv_hi_Temperature[GET_TWO];
...
static void run_time_process(void)
{
    ...
    if ((g_sys.status.dout_bitmap & (0x0001 << DO_FAN_BPOS)) != 0)
    {
        ...
        time_calc(&g_sys.status.run_time[DO_FILLTER_DUMMY_BPOS].low,
                  &g_sys.status.run_time[DO_FILLTER_DUMMY_BPOS].high);
    }
    //		// HAUWEI 风机运行时间扩展
    //		fans_run_time();
    u16_Sec++;
    ...
    {
        u16_Sec = 0;
        I2C_EE_BufWrite((uint8_t*)&g_sys.status.run_time, STS_REG_EE1_ADDR,
                        sizeof(g_sys.status.run_time));  // when, fan is working update eeprom every minite
        // bitmapset backup
        I2C_EE_BufWrite((uint8_t*)&team_local_inst.team_bitmap[TEAM_BITMAP_FINAL_OUT], STS_REG_EE2_ADDR,
                        sizeof(team_local_inst.team_bitmap[TEAM_BITMAP_FINAL_OUT]));
    }
    //		get_local_time(&now);
    //		if((now&0x0000007f) == 0x7f)
    //		{
    //				I2C_EE_BufWrite((uint8_t *)&g_sys.status.run_time,STS_REG_EE1_ADDR,sizeof(g_sys.status.run_time));
    ////when, fan is working update eeprom every minite
    //		}
    //    // bitmapset backup
    //    if((now & 0x0000007f) == 0x7f)
    //    {
    //        I2C_EE_BufWrite((uint8_t
    //        *)&team_local_inst.team_bitmap[TEAM_BITMAP_FINAL_OUT],STS_REG_EE2_ADDR,sizeof(team_local_inst.team_bitmap[TEAM_BITMAP_FINAL_OUT]));
    //    }

    //		rt_kprintf("[DO_FAN_BPOS]high= %x,[DO_FAN_BPOS]low= %x,[DO_RH1_BPOS]high= %x,[DO_RH1_BPOS]low=
    //%x,[DO_RH2_BPOS]high= %x,[DO_RH2_BPOS]low= %x,[0]high= %x,[0]low= %x,[0]high= %x ,[0]low= %x,[0]high= %x,[0]low=
        ...
        if (check_timeout > CHECK_TEAM_CONFIG_TIMEOUT)
        {
            if ((team_local_inst.team_config[TEAM_CONF_TEMP_DEADBAND] != g_sys.config.algorithm.temp_deadband) ||
                ...
                (team_local_inst.team_config[TEAM_CONF_FAN_FLOW_DIFF_DEADZONE] !=
                 g_sys.config.fan.flow_diff_deadzone) ||
                ...
                (team_local_inst.team_config[TEAM_CONF_FAN_FLOW_DIFF_DELAY] != g_sys.config.fan.flow_diff_delay))
            {
                ...
                //                                                              (g_sys.config.algorithm.ctrl_target_mode
                //                                                              == 0)?
                //                                                              g_sys.config.algorithm.return_air_temp:g_sys.config.algorithm.supply_air_temp,
                //                                                              g_sys.config.algorithm.hum_deadband,
                //                                                              g_sys.config.algorithm.hum_precision,
                //                                                              (g_sys.config.algorithm.ctrl_target_mode
                //                                                              == 0)?
                //                                                              g_sys.config.algorithm.return_air_hum:g_sys.config.algorithm.supply_air_hum,
                //                                                              g_sys.config.alarm[ACL_HI_TEMP_RETURN].alarm_param,
                //                                                              g_sys.config.alarm[ACL_LO_TEMP_RETURN].alarm_param,
                //                                                              g_sys.config.alarm[ACL_HI_HUM_RETURN].alarm_param,
                //                                                              g_sys.config.alarm[ACL_LO_HUM_RETURN].alarm_param);
