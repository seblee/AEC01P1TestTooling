...
static void top_fsm(void)
{
    ...
    switch (l_sys.t_fsm_state)
    {
        ...
        case (T_FSM_STATE_TEAM): {
            if (((g_sys.config.team.team_en == 0) &&
                 (g_sys.config.general.power_mode == 0)) ||               // power down and team disable
                ((g_sys.status.general.sys_error_bitmap & 0x0001) != 0))  // SYS_ERR_FATAL
            {
                l_sys.t_fsm_state = T_FSM_STATE_IDLE;
            }
            else if (((g_sys.config.general.power_mode == 1) &&
                      (g_sys.config.team.team_en == 0)) ||                          // power on and team disable
...
static void req_process(void)
{
    ...
    //								{
    //											if(((g_sys.config.humidifier.hum_en == 0)||(sys_get_mbm_online(POWER_MODULE_BPOS) ==
    //0))&&(target_hum_req <= -100))
    //											{
    //													target_temp_req =0;
    //													target_fan_req =0;
    ...
    //			 ((team_local_inst.team_config[TEAM_CONF_MODE] >> TEAM_CONF_TEAM_POWER_OFF_EN) & 0x0001))
    ////主机下发的故障是否关机参数
    //		{
    //						target_temp_req = l_sys.require[LOCAL_REQ][T_REQ];
    //						target_hum_req = l_sys.require[LOCAL_REQ][H_REQ];
