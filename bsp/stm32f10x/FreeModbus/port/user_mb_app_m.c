...
mbm_write_dev_reg EX_FAN_write_reg[EX_FAN_WRITE_CNT] = {
    {0x1037, 414, &g_sys.config.mbm_Conf.EX_FAN[0].Set_Speed_Mode.Data,
     &g_sys.config.mbm_Conf.EX_FAN[0].Set_Speed_Mode.Flag, &g_sys.status.mbm.EX_FAN[0].Set_Speed_Mode},
...
mbm_read_dev_reg EEV_read_reg[EEV_READ_CNT]    = {{0x78, 0x18, &g_sys.status.mbm.EEV.run_mode, 0x00},  // offset = 110
                                               {0x13, 0x0B, &g_sys.status.mbm.EEV.Set_Dehumidity_Supperheart, 0x18}};
mbm_write_dev_reg EEV_write_reg[EEV_WRITE_CNT] = {
    {21, 465, &g_sys.config.mbm_Conf.EEV.ntc_cali[0].Data, &g_sys.config.mbm_Conf.EEV.ntc_cali[0].Flag,
     &g_sys.status.mbm.EEV.Set_ntc_cali[0]},
    {22, 466, &g_sys.config.mbm_Conf.EEV.ntc_cali[1].Data, &g_sys.config.mbm_Conf.EEV.ntc_cali[1].Flag,
     &g_sys.status.mbm.EEV.Set_ntc_cali[1]},
    {23, 467, &g_sys.config.mbm_Conf.EEV.ai_cali[0].Data, &g_sys.config.mbm_Conf.EEV.ai_cali[0].Flag,
     &g_sys.status.mbm.EEV.Set_ai_cali[0]},
    {24, 468, &g_sys.config.mbm_Conf.EEV.ai_cali[1].Data, &g_sys.config.mbm_Conf.EEV.ai_cali[1].Flag,
     &g_sys.status.mbm.EEV.Set_ai_cali[1]},
    {25, 469, &g_sys.config.mbm_Conf.EEV.Set_Superheat[0].Data, &g_sys.config.mbm_Conf.EEV.Set_Superheat[0].Flag,
     &g_sys.status.mbm.EEV.Set_Superheat[0]},
    {26, 470, &g_sys.config.mbm_Conf.EEV.Set_Superheat[1].Data, &g_sys.config.mbm_Conf.EEV.Set_Superheat[1].Flag,
     &g_sys.status.mbm.EEV.Set_Superheat[1]},
    {27, 471, &g_sys.config.mbm_Conf.EEV.Set_Manual_Mode.Data, &g_sys.config.mbm_Conf.EEV.Set_Manual_Mode.Flag,
     &g_sys.status.mbm.EEV.Set_Manual_Mode},
    {28, 472, &g_sys.config.mbm_Conf.EEV.Set_Manual_Steps[0].Data, &g_sys.config.mbm_Conf.EEV.Set_Manual_Steps[0].Flag,
     &g_sys.status.mbm.EEV.Set_Manual_Steps[0]},
    {29, 473, &g_sys.config.mbm_Conf.EEV.Set_Manual_Steps[1].Data, &g_sys.config.mbm_Conf.EEV.Set_Manual_Steps[1].Flag,
     &g_sys.status.mbm.EEV.Set_Manual_Steps[1]},
};
...
const mbm_read_st mbm_read_table[MBM_DISCRETE_DEV_CNT] = {
    ...
    //		{  MBM_DEV_EX_FAN2_ADDR,         EX_FAN2_READ_CNT,    EX_FAN2_read_reg,  EX_FAN2_WRITE_CNT,
    // EX_FAN2_write_reg },//EX_FAN2;
...
void modbus_master_thread_entry(void* parameter)
{
    ...
    eStatus = eMBMasterInit(MB_RTU, UPORT_MBMASTER, 4800, MB_PAR_NONE);
        ...
        (mbm_dest_addr == MBM_DEV_EX_FAN_ADDR) || (mbm_dest_addr == MBM_DEV_EEV_ADDR))
    {
        ...
        switch (eMode)
        {
            ...
            case MB_REG_WRITE:
                ...
                //								if((mbm_dest_addr==MBM_DEV_EX_FAN_ADDR) ||
                //(mbm_dest_addr==MBM_DEV_EEV_ADDR)||(mbm_dest_addr==MBM_DEV_INV_ADDR))    //地址数据更新
                if ((mbm_dest_addr == MBM_DEV_EX_FAN_ADDR) || (mbm_dest_addr == MBM_DEV_EEV_ADDR))  //地址数据更新
                {
                    ...
                    {
                        if ((usAddress ==
                             mbm_read_table[mbm_dest_addr - MB_MASTER_DISCRETE_OFFSET].rd_pt[k].dev_reg_addr) &&
                    ...
                    {
                        iRegIndex =
                            mbm_read_table[mbm_dest_addr - MB_MASTER_DISCRETE_OFFSET].rd_pt[index].J_Offset_Addr;
                        while (usNRegs > 0)
                        {
                ...
                if((mbm_dest_addr==MBM_DEV_EEV_ADDR)||(mbm_dest_addr==MBM_DEV_INV_ADDR))//地址数据更新
                {
                        if(index < mbm_read_table[mbm_dest_addr - MB_MASTER_DISCRETE_OFFSET].reg_rd_cnt)
                        {
                                iRegIndex	=mbm_read_table[mbm_dest_addr -
                MB_MASTER_DISCRETE_OFFSET].rd_pt[index].J_Offset_Addr; while( usNRegs > 0 )
                                {
                                        pusRegHoldingBuf[iRegIndex] = *pucRegBuffer++ << 8;
                                        pusRegHoldingBuf[iRegIndex] |= *pucRegBuffer++;
                                        iRegIndex++;
                        ...
                        {
                                rt_kprintf("mbm_read_table[mbm_dest_addr -
                MB_MASTER_DISCRETE_OFFSET].reg_rd_cn_ERRO\n");
                        }
                }
                */
                else  //地址1-8数据更新
        ...
        if (((dev_init_bitmap_reg >> i) & 0x0001) == 1)
        {
            if ((i == MBM_DEV_EX_FAN_ADDR) || (i == MBM_DEV_EEV_ADDR))
            {
                for (j = 0; j < mbm_read_table[i - MB_MASTER_DISCRETE_OFFSET].reg_rd_cnt; j++)
                {
                    //	rt_thread_delay(50);
                    //											errorCode =
                    // eMBMasterReqReadHoldingRegister((i+1),mbm_read_table[i-MB_MASTER_DISCRETE_OFFSET].rd_pt[j].dev_reg_addr,1,MBM_RESPONSE_DELAY);
                    errorCode = eMBMasterReqReadHoldingRegister(
                        (i + 1), mbm_read_table[i - MB_MASTER_DISCRETE_OFFSET].rd_pt[j].dev_reg_addr,
...
static void mbm_fsm_update(sys_reg_st* gds_ptr, mbm_dev_st* mbm_dev_inst)
{
    ...
    switch (mbm_fsm_cstate)
    {
        ...
        case (MBM_FSM_SYNC): {
            ...
            if (((mbm_dev_inst->bitmap.update) ^ (u16mb_comp)) ==
        ...
        case (MBM_FSM_UPDATE): {
            ...
            mbm_dev_inst->Write_Enable = TRUE;
            if ((mbm_dev_inst->timeout.update >= MBM_UPDATE_TIMEOUT_THRESHOLD) ||
                (u16mb_comp != mbm_dev_inst->bitmap.update))  // if update err count timeout, swich to sync state
            {
    ...
    if ((mbm_dev_inst->errcnt.update == 0) || (mbm_dev_inst->errcnt.update >= 5) ||
        (mbm_dev_inst->bitmap.update == u16mb_comp))
    {
...
void mbm_sts_update(sys_reg_st* gds_ptr)
{
    ...
    mbm_dev_update_bitmap =
        mbm_dev_inst.bitmap
            .update;  // get modbus update bitmap which could be used to determin which glabal regsiter to update
    //		mbm_dev_init_bitmap = mbm_dev_inst.bitmap.init;	//get modbus init bitmap which could be used to determin
    // which glabal regsiter to update 		mbm_dev_init_bitmap = (g_sys.config.dev_mask.mb_comp&0x7FFF);
    ...
    //		for(i=MB_DEV_TH_ADDR_START;i<=MB_DEV_TH_ADDR_END;i++)	//copy modbus master device register to global
    // register, temp and hum sensor daq
    //		{
    ...
    //			{
    //					if(((mbm_dev_update_bitmap >> MBM_DEV_H_ADDR)&0x0001) != 0)	//copy modbus master device register
    // to global register, humidifier daq
    //					{
    //							//有加湿电流和注水的情况下不更新
    ...
    //							{
    //
    //									gds_ptr->status.mbm.hum.conductivity =
    // usMRegHoldBuf[MBM_DEV_H_ADDR][MBM_DEV_H_REG_CONDUCT_ADDR];
    //
    //							}
    //							gds_ptr->status.mbm.hum.dev_sts =
    // usMRegHoldBuf[MBM_DEV_H_ADDR][MBM_DEV_H_REG_STATUS_ADDR]; gds_ptr->status.mbm.hum.hum_current =
    // usMRegHoldBuf[MBM_DEV_H_ADDR][MBM_DEV_H_REG_HUMCUR_ADDR]; gds_ptr->status.mbm.hum.water_level =
    // usMRegHoldBuf[MBM_DEV_H_ADDR][MBM_DEV_H_REG_WT_LV_ADDR];
    //					}
    //					else	//if device is not initialized, all date reset to 0
    //					{
    ...
    {
        if (((mbm_dev_update_bitmap >> MBM_DEV_PT_ADDR) & 0x0001) !=
            0)  // copy modbus master device register to global register, power daq
        {
            u8ComErr                             = 0;
    ...
    //            gds_ptr->status.mbm.pwr[0].p_cur[0], gds_ptr->status.mbm.pwr[0].p_cur[1],
    //            gds_ptr->status.mbm.pwr[0].p_cur[2], gds_ptr->status.mbm.pwr[0].pe_bitmap);
    //	电源板
    if (((mbm_dev_init_bitmap >> MBM_DEV_P_ADDR) & 0x0001) != 0)
    {
        if (((mbm_dev_update_bitmap >> MBM_DEV_P_ADDR) & 0x0001) !=
            0)  // copy modbus master device register to global register, power daq
        {
            u8ComErr                             = 0;
    ...
    // rt_kprintf("dev_sts1= %x,pa_volt1= %d,p_cur[0]1= %d,p_cur[1]1= %d,p_cur[2]1= %d,pe_bitmap= %d\n",
    //            gds_ptr->status.mbm.pwr[1].dev_sts, gds_ptr->status.mbm.pwr[1].pa_volt,
    //            gds_ptr->status.mbm.pwr[1].p_cur[0], gds_ptr->status.mbm.pwr[1].p_cur[1],
    //            gds_ptr->status.mbm.pwr[1].p_cur[2], gds_ptr->status.mbm.pwr[1].pe_bitmap);
    ...
    //					{
    //							if(((mbm_dev_update_bitmap >> mbm_read_table[j].mbm_addr)&0x0001) != 0)	//copy modbus
    //master device register to global register, power daq
    //							{
    //									for(k=0;k < mbm_read_table[j].reg_rd_cnt;k++)
    //									{
    //											for(h=0;h<mbm_read_table[j].rd_pt[k].NRegs;h++)
    //											{
    //													*(mbm_read_table[j].rd_pt[k].data_ptr+h) = usMRegHoldBuf[
    // mbm_read_table[j].mbm_addr][mbm_read_table[j].rd_pt[k].J_Offset_Addr+h];
    //											}
    //									}
    ////									//test
    ////
    /// rt_kprintf("Fre=%d,M_Voltage=%d,Buf_Voltage=%d\n",g_sys.status.mbm.EX_FAN.Fre,g_sys.status.mbm.EX_FAN.M_Voltage,usMRegHoldBuf[
    /// MBM_DEV_EX_FAN_ADDR][1]);
    ...
    //														if(*(mbm_read_table[j].w_pt[k].conf_ptr) !=
    //*(mbm_read_table[j].w_pt[k].status_ptr))
    //														{
    //															mbm_send_data.mbm_addr = mbm_read_table[j].mbm_addr + 1;
    //															mbm_send_data.reg_addr =
    // mbm_read_table[j].w_pt[k].dev_reg_addr; mbm_send_data.reg_value=
    // *(mbm_read_table[j].w_pt[k].conf_ptr); 															mbm_send_data.mbm_NRegs = 1;
    // mbm_send_data.mbm_fun_code = MB_WRITE_SINGLE;
    //															if(fifo8_push(&mbm_data_fifo,(uint8_t*)&mbm_send_data)
    //== 0)
    //															{
